...
description: |
  Authoritative reference for the io.xh:hoist-core Grails/Groovy framework AND the install/upgrade authority for its MCP server + CLI tools (`hoist-core-mcp`, `hoist-core-docs`, `hoist-core-symbols`) -- ALWAYS use this skill whenever the user wants to write, modify, debug, or explain Groovy/Java backend code that touches hoist-core, OR wants to install/refresh/wire up/upgrade the hoist-core MCP+CLI surface in a project.
  ...
  Never improvise hoist-core class names, method signatures, annotations, or conventions -- hoist-core evolves and stale guesses produce real bugs. Consult the reference tools (MCP `mcp__hoist-core__*` or CLI `./bin/hoist-core-docs` / `./bin/hoist-core-symbols`) first.
