...
description: |
  Authoritative reference for the io.xh:hoist-core Grails/Groovy framework AND the install/upgrade authority for its MCP server + CLI tools (`hoist-core-mcp`, `hoist-core-docs`, `hoist-core-symbols`) -- use when about to write, modify, debug, or explain Groovy/Java backend code that touches hoist-core, or when the user wants to install/refresh/wire up/upgrade the hoist-core MCP+CLI surface in a project.
  Why this matters: hoist-core's base classes (`BaseService`, `BaseController`) and infrastructure types (`Cache`, `CachedValue`, `Timer`, `ClusterService`, `ConfigService`) have specific conventions for injection, cluster-awareness, and config that don't match plain Spring/Grails idioms. Guessing produces code that compiles, runs fine single-node, then misbehaves under cluster failover -- cache invalidation that doesn't propagate, configs read once at startup that never refresh, timers that double-fire across nodes, scheduled tasks that miss their slot. These failures rarely surface in local dev.
  TRIGGER when the user mentions: any hoist-core symbol (`BaseService`, `BaseController`, `RestController`, `HoistUser`, `Cache`, `CachedValue`, `Timer`, `Filter`, `ConfigService`, `ClusterService`, `MonitorService`, `JSONClient`, `LogSupport`); a hoist-core concept (caching, cluster-aware, distributed cache, config-driven, monitoring, scheduled task, JSON client, framework conventions); a service/controller file under `grails-app/` or `src/` for non-trivial authoring; how to do a backend task "the framework way" / "using our conventions" / "the Hoist way"; installing, setting up, wiring, refreshing, or upgrading the hoist-core MCP server, CLI tools, `./bin/hoist-core-*` launchers, `installHoistCoreTools` Gradle task, or `.mcp.json` entries; `hoistCoreVersion` (especially after bumping it); "hoist-core", "io.xh.hoist", or "our framework" in any Groovy/Java backend context.
  SKIP for plain Spring/Spring Security work without hoist-core (`@PreAuthorize`, raw `@Controller`, JPA/Hibernate annotations on plain domain classes), generic Grails work that doesn't import `io.xh.hoist`, Liquibase/Flyway migrations, and build-tooling-only changes (`build.gradle` dependency adds, `gradle.properties` version bumps) that don't touch hoist-core.
  Reach for the reference tools (MCP `mcp__hoist-core__*` or CLI `./bin/hoist-core-docs` / `./bin/hoist-core-symbols`) first.
